...
function updateReferenceDisplay() {
    ...
    if (referenceCount) {
        referenceCount.textContent = `Uploaded: ${referenceFiles.length}/4`;
    }
}
...
if (referenceDropzone) {
    ...
    referenceDropzone.addEventListener('drop', (e) => {
        ...
        const remainingSlots = 4 - referenceFiles.length;

        if (files.length > remainingSlots) {
            showModal(`Please upload only ${remainingSlots} more file(s). Maximum is 4 files.`);
            const filesToAdd = files.slice(0, remainingSlots);
            referenceFiles = [...referenceFiles, ...filesToAdd];
        } else {
            referenceFiles = [...referenceFiles, ...files];
...
if (verifyBtn) {
    verifyBtn.addEventListener('click', async () => {
        ...
        if (referenceFiles.length < 4) {
            showModal(`Please upload ${4 - referenceFiles.length} more reference signature(s). You need 4 reference images.`);
            return;
        }
